...
class ChessState {
	...
	static class ChessMoveIterator
	{
	 ...
	 static int alphaBetaPruning(ChessState s,int depth,int alpha,int beta,boolean maximazingPlayer,Random rand){
		ChessMoveIterator it = s.iterator(maximazingPlayer);
		...
		if(maximazingPlayer){
			...
			while(it.hasNext()){
				m = it.next();
				ChessState temp = new ChessState(s);
				temp.move(m.xSource,m.ySource,m.xDest,m.yDest);
				value = Math.max(value,alphaBetaPruning(temp,depth-1,alpha,beta,false,rand));
			...
			while(it.hasNext()){
				...
				ChessState temp = new ChessState(s);
				temp.move(m.xSource,m.ySource,m.xDest,m.yDest);
				value = Math.min(value,alphaBetaPruning(temp,depth-1,alpha,beta,true,rand));
	...
	public static void main(String[] args) {
		...
		do{
			s.printBoard(System.out);
			System.out.println();
			move = new ChessState.ChessMove();
			if(turn && a!=0){
				int choose = Integer.MIN_VALUE;
				ChessMoveIterator it = s.iterator(turn);
				m = new ChessState.ChessMove();
				while(it.hasNext()){
					ChessState temp = new ChessState(s);
					m = it.next();
					temp.move(m.xSource,m.ySource,m.xDest,m.yDest);
					int h = alphaBetaPruning(temp,a-1,Integer.MIN_VALUE,Integer.MAX_VALUE,!turn,rand);
					if(h>choose){
						move = m;
						choose = h;
					}
				...
				while(it.hasNext()){
					...
					m = it.next();
					temp.move(m.xSource,m.ySource,m.xDest,m.yDest);
					int h = alphaBetaPruning(temp,b-1,Integer.MIN_VALUE,Integer.MAX_VALUE,!turn,rand);
					if(h<choose){
						move = m;
						choose = h;
					}
				...
				do {
					...
					String humanMove = in.nextLine();
					if (humanMove.equals("q")) {
						System.out.println("Game ended");
						System.exit(-1);
					}
					move.xSource = humanMove.charAt(0) - 'a';
					move.ySource = (humanMove.charAt(1) - '0') - 1;
					move.xDest = humanMove.charAt(2) - 'a';
					move.yDest = (humanMove.charAt(3) - '0') - 1;
					isvalid = s.isValidMove(move.xSource,move.ySource,move.xDest,move.yDest);
					if(isvalid == false){
						System.out.println("invalid move, try again:");
					}
					if(move.xSource > 8 || move.xSource < 0 || move.xDest>8 || move.yDest <0){
			...
			turn = !turn;
		}while(!s.move(move.xSource,move.ySource,move.xDest,move.yDest));
		if(!turn)
